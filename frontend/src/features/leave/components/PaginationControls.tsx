...
export function PaginationControls({
  ...
  const totalPages = Math.max(1, Math.ceil(totalItems / LEAVE_PAGE_SIZE));
  const startItem = totalItems === 0 ? 0 : (currentPage - 1) * LEAVE_PAGE_SIZE + 1;
  const endItem = Math.min(currentPage * LEAVE_PAGE_SIZE, totalItems);

  if (totalItems <= LEAVE_PAGE_SIZE) return null;
