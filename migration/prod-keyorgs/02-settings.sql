-- Same script as ../prod-settings/02-settings.sql, pointed at _mig_orgmap_keyorgs.
-- Kept a verbatim copy rather than a rewrite: that one ran against these exact tables
-- on 2026-09-18 and was field-diffed to 0 mismatches.
--
...
JOIN altomatehr._mig_orgmap_keyorgs m ON m.v1_id = o.id
...
JOIN altomatehr._mig_orgmap_keyorgs m ON m.v1_id = t.organizationId
ON DUPLICATE KEY UPDATE
...
JOIN altomatehr._mig_orgmap_keyorgs m ON m.v1_id = s.organizationId
ON DUPLICATE KEY UPDATE
...
JOIN altomatehr._mig_orgmap_keyorgs m ON m.v1_id = c.organizationId
ON DUPLICATE KEY UPDATE
