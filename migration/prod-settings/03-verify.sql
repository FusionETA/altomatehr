-- Verification for the hr_prod -> altomatehr settings migration. All checks must return 0.
...
UNION ALL SELECT 'orphan PayrollCompanyInfos.OrganizationId', COUNT(*) FROM altomatehr.PayrollCompanyInfos x
  LEFT JOIN altomatehr.Organizations o ON o.Id = x.OrganizationId WHERE o.Id IS NULL
UNION ALL SELECT 'org name mismatch vs hr_prod', COUNT(*) FROM altomatehr._mig_orgmap m
...
UNION ALL SELECT 'bad Addons format', COUNT(*) FROM altomatehr.Organizations o
  JOIN altomatehr._mig_orgmap m ON m.v2_id = o.Id COLLATE utf8mb4_unicode_ci
  WHERE o.Addons NOT IN ('', 'expense_claim,clock', 'clock,expense_claim')
UNION ALL SELECT 'leavetype count drift', COUNT(*) FROM (
    SELECT m.v2_id id, COUNT(*) n FROM hr_prod.LeaveType t JOIN altomatehr._mig_orgmap m ON m.v1_id = t.organizationId GROUP BY 1) src
...
UNION ALL SELECT 'project count drift', COUNT(*) FROM (
    SELECT m.v2_id id, COUNT(*) n FROM hr_prod.XeroProject x JOIN altomatehr._mig_orgmap m ON m.v1_id = x.organizationId GROUP BY 1) src
